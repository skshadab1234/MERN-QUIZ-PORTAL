  ...
  useEffect(() => {
    settingsData.then(res => {
      ...
      if (res.testGoing == false) {
        ...
        var DateDisplay = 'Test will Start'
        if(datenow.getDate()+" "+monthNames[datenow.getUTCMonth()]+","+datenow.getFullYear() == res.testDate){
            DateDisplay += ' Today at '+res.testTime
            if(datenow.getHours()+":"+datenow.getMinutes() > res.testTime) {
              DateDisplay = 'Test Ended'
            }
        }else{
          DateDisplay += '  On '+res.testDate+" at "+res.testTime
        }
    ...
    try {
      ...
      if (!data.status === 200) {
        ...
        if (data.testOn == 'false') {
          settestRejection("Already Test Submitted")
        } 
            ...
            testRejection == '' ?
              <>
                ...
                {showModal ? (
                  ...
                  <div className='flex h-80 justify-center place-items-center'>
                    <h1 className='text-xl md:text-5xl text-white font-extralight	tracking-wide	'>{testRejection}</h1>
