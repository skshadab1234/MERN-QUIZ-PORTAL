...
const Homepage = () => {
  ...
  var timer = setInterval(function () {
    ...
    if(days >  0 || hours >  0 || minutes >  0 || seconds > 0){
      ...
      if (typeof window !== "undefined") {
        document.getElementById("timer__section").classList.remove("hidden")
        document.getElementById("button_proceed").classList.add('hidden')
      }
    }
  }, 1000);
  ...
  return (
    <div className='h-full'>
      { 
        IsLoading ? <div role="status" className='flex justify-center'>
      ...
      {/* Timer Section  */}
      <div id="timer__section" className='hidden container flex justify-center'>
        <div className='p-3 grid grid-cols-2 gap-6 md:grid-cols-4 md:gap-3'>
          ...
          <div className={styles.timerDiv}>
            <h1><span id="hours">{hours}</span>  <span className='text-base	font-light'>Hours</span> </h1>
          </div>
          <div className={styles.timerDiv}>
